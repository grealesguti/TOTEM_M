function [u,lambda,iter,Aout,DeltaSout] = arc_length_Crisfield(functn,u0,varargin)
...
optargs = {'sph' 2 0 1 12 4 5e-5 1 1e-4};
...
for p=1:nmax
    ...
    while i<imax
        ...
        %    end
        %end
        KT_Distr = distributed(KT); 
        R_Distr=distributed(R);
        F_Distr=distributed(F);

        DeltauRi=-KT_Distr\R_Distr; % eq.(30) in [3]
        DeltauFi=KT_Distr\F_Distr; % eq.(31) in [3]
        if i==0
            if p==1
            ...
            if (b^2-a*c) > 0 % two real roots
                ...
                upi1=[ui1(:,1)-up,ui1(:,2)-up]; % eq.(29) in [3]
                cosines=[upi'*upi1(:,1)/abs(upi'*upi1(:,1)),upi'*upi1(:,2)/abs(upi'*upi1(:,2))]; % eq.(33) in [3]
            ...
            lambdai=lambda0+Deltalambdai; % eq.(4) in [3]
            % convergence test
            fprintf('*Arc-len it i %i/%i Convergence %1.2e/%1.0e.\n',i,imax,max(abs(Deltaui./ui)),tol);
            if max(abs(Deltaui./ui)) < tol
                break;
            end
        end
