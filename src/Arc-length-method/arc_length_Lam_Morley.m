function [p_out,lambda_out,iter_out] = arc_length_Lam_Morley(functn,q,p0,varargin)
...
while IINCS<=maxIINCS
    ...
    Deltalambdapr=lambda-lambda0; % store
    Deltappr=p-p0;
    Ipr=IITER; % store
    p_out(:,IINCS)=p;
...
while isreal(eta)
    ...
    if b_x^2-a_x*c_x<0
        ...
        prod1=Deltap1'*Deltappr; % text after eq.(12d) in [6]
        prod2=Deltap2'*Deltappr; % text after eq.(12d) in [6]
        x=x1*(prod1>prod2)+x2*(prod2>prod1); % text after eq.(12d) in [6]
        Deltap=Deltap1*(prod1>prod2)+Deltap2*(prod2>prod1); % text after eq.(12d) in [6]
        Deltalambda=Deltalambda-g+x; % eq.(9c) in [6]
        return;
    end
end
end
