...
if any(strcmpi(varargin, 'PRECISION'))
    precision = int2str(varargin{find(strcmpi(varargin, 'PRECISION'))+1});
else
    precision = '4';
end
switch upper(dataType)
    ...
    case {'STRUCTURED_GRID','UNSTRUCTURED_GRID'}
        ...
        if sum(size(x)==size(y) & size(y)==size(z))~=length(size(x))
            error('Input POINT dimesions do not match')
        end
        n_elements = numel(x);
        ...
        if ~binaryflag
            spec = ['%.',(precision),'f ','%.',precision,'f ','%.',precision,'f \n'];
        ...
        if cidx~=0
            for ii = 1:length(cidx(:,1))
                ...
                nodecol=zeros(ncells,1);
                nodecol(:,1)=nodepercell;
                output=[nodecol,cells-1];
                if ~binaryflag
                    spec0=['%10i '];
                    spec=['%10i '];
                    for jj=1:nodepercell
                        spec=append(spec,spec0);
                    end
                    spec=append(spec,'\n');
                    fprintf(fid,spec, output');
                else
                ...
                if ~binaryflag
                    prncells=['CELL_TYPES ',int2str(ncells), '\n'];
                    fprintf(fid,prncells);
                    fprintf(fid,'%10i\n', celltypecol);
                else
                    prncells=['CELL_TYPES ',int2str(ncells), '\n'];
                    fprintf(fid,prncells);
                    fwrite(fid, celltypecol, 'float', 'b');
                end

            end
        end
        cdidx = find(strcmpi(varargin,'CELL_DATA'));
        if cdidx~=0
            for ii = 1:length(cdidx(:,1))
                celldata=[varargin{ cdidx(ii) + 1 }(:,:)']';
                if length(celldata(:,1))~=ncells
                    error('Input CELL_DATA dimesions do not match')
                end
                celldata=[varargin{ cdidx(ii) + 1 }(:,:)'];
                dataname=[varargin{ cdidx(ii) + 2 }];
                prncells=['\nCELL_DATA ',int2str(ncells),'\n'];fprintf(fid,prncells);
                prncells=['SCALARS ',dataname,' float ',int2str(length(celldata(:,1))),' \n'];fprintf(fid,prncells);
                prncells=['LOOKUP_TABLE ',dataname,'\n'];fprintf(fid,prncells);
                if ~binaryflag
                    spec0=[' %e '];
                    spec=[' %e '];
                    for jj=1:length(celldata(:,1))-1
                        spec=append(spec,spec0);
                    end
                    spec=append(spec,'\n');
                    fprintf(fid,spec, celldata);
                else
                    fwrite(fid, celldata, 'float', 'b');
                end
            end
        end
        ...
        if vidx~=0
            for ii = 1:length(vidx(:,1))
                vecdata=[varargin{ vidx(ii) + 1 }(:,:)']';
                if length(vecdata(:,1))~=ncells
                    error('Input CELL_DATA dimesions do not match')
                end
                vecdata=[varargin{ vidx(ii) + 1 }(:,:)'];
                dataname=[varargin{ vidx(ii) + 2 }];
                prncells=['VECTORS ',dataname,' float ',' \n'];fprintf(fid,prncells);
                prncells=['LOOKUP_TABLE ',dataname,'\n'];fprintf(fid,prncells);

                if ~binaryflag
                    spec = ['%.',(precision),'f ','%.',precision,'f ','%.',precision,'f \n'];
                    fprintf(fid,spec0, vecdata);
                else
                    fwrite(fid, vecdata, 'float', 'b');
                end
            end
        end
        ...
        if pidx~=0
            for ii = 1:length(pidx(1,:))
                pointdata=[varargin{ pidx(ii) + 1 }(:,:)']';
                if length(pointdata(:,1))~=length((x))
                    error('Input POINT_DATA dimesions do not match')
                end
                dataname=[varargin{ pidx(ii) + 2 }];
                prncells=['\nPOINT_DATA ',int2str(length((x))),'\n'];fprintf(fid,prncells);
                prncells=['SCALARS ',dataname,' float ',int2str(length(pointdata(1,:))),' \n'];fprintf(fid,prncells);
                prncells=['LOOKUP_TABLE ',dataname,'\n'];fprintf(fid,prncells);
                if ~binaryflag
                    spec0=[' %e '];
                    spec=[' %e '];
                    for jj=1:length(pointdata(1,:))-1
                        spec=append(spec,spec0);
                    end
                    spec=append(spec,'\n');
                    fprintf(fid,spec, pointdata');
                else
                    fwrite(fid, pointdata, 'float', 'b');
                end
            end
        end
